...
void HDF5Exporter::exportParticles(int step, double time, const std::vector<MPMParticle>& particles) {
    (void)time;
#ifdef HAS_HDF5
    if (!m_file) {
        createFile("simulation.h5");
...
void HDF5Exporter::exportMetrics(int step, double time, double maxStress, double maxTemp) {
    appendTimeSeries("max_stress", time, maxStress);
    appendTimeSeries("max_temperature", time, maxTemp);
}

void HDF5Exporter::exportDetailedMetrics(int step, double time,
                                         double workpieceStress, double workpieceTemp,
                                         double toolStress, double toolTemp,
                                         double coolantTemp) {
    (void)step;
    appendTimeSeries("workpiece_max_stress", time, workpieceStress);
    appendTimeSeries("workpiece_max_temperature", time, workpieceTemp);
    appendTimeSeries("tool_max_stress", time, toolStress);
    appendTimeSeries("tool_max_temperature", time, toolTemp);
    appendTimeSeries("coolant_max_temperature", time, coolantTemp);
    exportMetrics(step, time,
                  std::max(workpieceStress, toolStress),
                  std::max(std::max(workpieceTemp, toolTemp), coolantTemp));
}
