...
void ThreadingStrategy::connectSolvers(MPMSolver* sph, FEMSolver* fem, 
...
void ThreadingStrategy::updateConditions(const MachineState& state, double dt) {
    ...
    m_conditions.feedRate = pitch * m_adaptiveFeedMultiplier;  // m/rev (feed = pitch for threading)
...
MachiningOutput ThreadingStrategy::computeOutput() const {
    ...
    double pitch = m_threadPitchMm / 1000.0;
    double R = fmax(m_majorDiameterMm, 1e-6) / 2000.0;
...
void ThreadingStrategy::applyEdgeSubgridCorrections() {
    if (!m_edgeSubgrid || !m_edgeSubgrid->isEnabled()) return;
    double h_eq = 0.0;
    if (m_currentPass > 0) {
        double currentDepth = getCurrentPassDepth();
        double prevDepth = (m_currentPass > 1) ? m_passDepths[m_currentPass - 2] : 0;
        double incDepth = currentDepth - prevDepth;
        double alpha = getFormHalfAngle();
        double chipArea = 2.0 * incDepth * incDepth * std::tan(alpha);
        h_eq = chipArea / (2.0 * incDepth / std::cos(alpha));
    }
    if (h_eq <= 0) return;
    auto result = m_edgeSubgrid->compute(
        h_eq, m_specificCuttingForce * 0.5,
        m_conditions.cuttingSpeed,
        m_currentPass > 1 ? 200.0 : 25.0,
        880e6, 113.8e9, 600e9);
...
void ThreadingStrategy::applyBUECorrections() {
...
void ThreadingStrategy::applyChatterModulation(double dt) {
