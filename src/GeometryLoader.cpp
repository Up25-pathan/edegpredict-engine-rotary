...
void GeometryLoader::alignToTip(Mesh& mesh, int alignAxis) {
    ...
    // Step 2: Compute centroid for cross-section centering
    double n = static_cast<double>(mesh.nodes.size());
    double centroidX = sumX / n;
    double centroidY = sumY / n;
    double centroidZ = sumZ / n;
    
    // Step 3: Build translation so tip = (0,0,0) and tool lies on that axis
    Vec3 offset;
    switch (alignAxis) {
        case 0: // Tool points along X
            offset = Vec3(-minAlignCoord, -centroidY, -centroidZ);
            break;
        case 1: // Tool points along Y
            offset = Vec3(-centroidX, -minAlignCoord, -centroidZ);
            break;
...
void GeometryLoader::voxelizeToTetMesh(Mesh& mesh, double elementSize, int maxNodes) {
    ...
    auto isInside = [&](const Vec3& p) -> bool {
        Vec3 rays[3] = {
            Vec3(1.0, 1.2345e-4, 3.4567e-5).normalized(),
            Vec3(1.2345e-4, 1.0, 3.4567e-5).normalized(),
            Vec3(3.4567e-5, 1.2345e-4, 1.0).normalized()
        };
        int insideCount = 0;
