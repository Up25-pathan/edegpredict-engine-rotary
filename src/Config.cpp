...
void Config::parseJson(const json& j) {
    ...
    if (j.contains("simulation_parameters")) {
        ...
        m_simulation.fidelityMode = sp.value("fidelity_mode", "standard");
        ...
        m_simulation.handoffGapMm = sp.value("handoff_gap_mm", 0.1);
        m_simulation.dynamicReconstruction = sp.value("dynamic_reconstruction", false);
        m_simulation.runToTargetDepth = sp.value("run_to_target_depth", true);
        m_simulation.targetDepthToleranceMm = sp.value("target_depth_tolerance_mm", 0.02);
        m_simulation.maxAutoExtendedSteps = sp.value("max_auto_extended_steps", 50000000);

        // Adaptive Mass Scaling (AMS)
        m_simulation.amsEnabled = sp.value("ams_enabled", true);
        m_simulation.amsTargetTimeStep = sp.value("ams_target_time_step_s", 1.0e-7);
        m_simulation.amsMaxScalingFactor = sp.value("ams_max_scaling_factor", 500.0);
        m_simulation.amsFrequencyDomainEnabled = sp.value("ams_frequency_domain_enabled", true);
        m_simulation.amsFrequencyTarget = sp.value("ams_frequency_target_hz", 0.0);
        m_simulation.amsSpectralRadiusLimit = sp.value("ams_spectral_radius_limit", 0.99);
        m_simulation.amsAdaptiveCFLMin = sp.value("ams_adaptive_cfl_min", 0.05);
        m_simulation.amsAdaptiveCFLMax = sp.value("ams_adaptive_cfl_max", 0.5);
        m_simulation.subCyclingSteps = sp.value("sub_cycling_steps", 8);
        m_simulation.contactThermalEveryNSubSteps = sp.value("contact_thermal_every_n_sub_steps", 4);
        m_simulation.thermalTimeConstant = sp.value("thermal_time_constant_s", 5.0);
        m_simulation.maxThermalIterations = sp.value("max_thermal_iterations", 8);
        m_simulation.thermalConvergenceTolerance = sp.value("thermal_convergence_tolerance_c", 0.5);
        m_simulation.enableThermalConvergenceLogging = sp.value("enable_thermal_convergence_logging", true);
    }
    ...
    if (j.contains("material_properties")) {
        ...
        m_material.poissonsRatio = mat.value("poissons_ratio", 0.34);
        ...
        // T-dependent property tables (arrays of [TÂ°C, value] pairs)
        auto parseTable = [](const json& j, const std::string& key) {
            std::vector<std::array<double, 2>> table;
            if (j.contains(key) && j[key].is_array()) {
                for (const auto& entry : j[key]) {
                    if (entry.is_array() && entry.size() >= 2) {
                        table.push_back({entry[0].get<double>(), entry[1].get<double>()});
                    }
                }
            }
            return table;
        };
        m_material.thermalConductivityTable = parseTable(mat, "thermal_conductivity_table");
        m_material.specificHeatTable = parseTable(mat, "specific_heat_table");
    ...
    if (j.contains("tool_material")) {
        ...
        auto parseToolTable = [](const json& j, const std::string& key) {
            std::vector<std::array<double, 2>> table;
            if (j.contains(key) && j[key].is_array()) {
                for (const auto& entry : j[key]) {
                    if (entry.is_array() && entry.size() >= 2) {
        ...
        m_toolMaterial.thermalConductivityTable = parseToolTable(tm, "thermal_conductivity_table");
        m_toolMaterial.specificHeatTable = parseToolTable(tm, "specific_heat_table");
    ...
    if (j.contains("sph_parameters")) {
        ...
        m_sph.gasStiffness = sp.value("gas_stiffness", 35e6);
        ...
        m_sph.numericalMassScalingFactor =
            std::max(1.0, sp.value("numerical_mass_scaling_factor", 1.0));
        
        // Damage/Chip Separation parameters â€” use Steel 4140 JC failure as defaults
    ...
    if (j.contains("fem_parameters")) {
        ...
        m_fem.poissonsRatio = fp.value("poissons_ratio", 0.22);
        m_fem.density = fp.value("density_kg_m3", 14500.0);
        m_fem.dampingRatio = fp.value("damping_ratio", 0.1);
        m_fem.maxNodes = fp.value("max_nodes", 50000);
        m_fem.maxElements = fp.value("max_elements", 200000);
    ...
    if (j.contains("cfd_parameters")) {
        ...
        m_cfd.chipFluidCouplingEnabled = cp.value("chip_fluid_coupling", true);
    }
...
void Config::validate() {
    ...
    if (m_filePaths.toolGeometry.empty()) {
        std::cerr << "[Config] WARNING: file_paths.tool_geometry is empty. "
                  << "Using parametric tool mesh fallback." << std::endl;
    ...
    if (m_filePaths.gcodeFile.empty()) {
        std::cerr << "[Config] WARNING: file_paths.gcode_file is empty. "
                  << "Running without G-Code toolpath control." << std::endl;
    ...
    // Validate simulation step budget
    ...
    // =========================================================================
    // NUMERIC RANGE VALIDATION (Fix 8: expanded to catch crash-causing inputs)
    // =========================================================================
    ...
    if (m_machining.rpm < 1.0) {
        m_errors.push_back("machining_parameters.rpm must be >= 1 (zero causes division by zero)");
        m_isValid = false;
    }
    if (m_sph.smoothingRadius <= 0) {
        m_errors.push_back("sph_parameters.smoothing_radius_m must be > 0 (zero causes NaN)");
        m_isValid = false;
    }
    if (m_machining.depthOfCutMm < 0) {
        m_errors.push_back("machining_parameters.depth_of_cut_mm cannot be negative");
        m_isValid = false;
    }
    if (m_sph.maxParticles < 100) {
        m_errors.push_back("sph_parameters.max_particles must be >= 100");
        m_isValid = false;
    }
    ...
    if (m_isValid) {
        ...
        if (m_simulation.runToTargetDepth && m_machining.depthOfCutMm > 0.0) {
            std::cout << "  Depth target: " << m_machining.depthOfCutMm
                      << " mm (auto-extend enabled)" << std::endl;
        }
        if (m_sph.numericalMassScalingFactor > 1.0) {
            std::cout << "  MPM mass scaling: " << m_sph.numericalMassScalingFactor
                      << "x (thermal density remains physical)" << std::endl;
        }
        if (m_cfd.enabled) {
