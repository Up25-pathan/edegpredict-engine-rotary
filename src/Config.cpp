    ...
    if (j.contains("machine_setup")) {
        ...
        if (ms.contains("work_offset_G54")) {
            ...
            for (size_t i = 0; i < std::min(offsets.size(), (size_t)3); ++i) {
                m_machineSetup.workOffsetG54[i] = offsets[i] / 1000.0; // mm to m
            }
        }
        ...
        std::cout << "[Config] Machine setup loaded:"
                  << " G54=[" << m_machineSetup.workOffsetG54[0]*1000 
                  << "," << m_machineSetup.workOffsetG54[1]*1000 
                  << "," << m_machineSetup.workOffsetG54[2]*1000 << "]mm"
