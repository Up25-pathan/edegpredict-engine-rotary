...
void Config::parseJson(const json& j) {
    ...
    if (j.contains("material_properties")) {
        ...
        // T-dependent property tables (arrays of [T°C, value] pairs)
...
void Config::validate() {
    ...
    // =========================================================================
    // Tool geometry: empty is allowed — main.cpp will generate a parametric
    // mesh fallback.  Only flag an error if a path is explicitly given but
    ...
    // G-Code file: empty is allowed — the engine will run with manual kinematics
    // (no toolpath interpolator).  Only flag an error if a path is explicitly
    // given but does not exist on disk.
    if (m_filePaths.gcodeFile.empty()) {
