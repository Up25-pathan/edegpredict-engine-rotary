...
void CFDSolverGPU::solvePressure() {
    ...
    // Allocate residual buffer once (reused across calls)
    static float* d_residual = nullptr;
    if (!d_residual) CUDA_CHECK(cudaMalloc(&d_residual, sizeof(float)));
    ...
    for (int iter = 0; iter < m_params.maxPressureIters; ++iter) {
        ...
        // Check convergence every 10 iterations (avoid per-step sync overhead)
        if ((iter + 1) % 10 == 0) {
            CUDA_CHECK(cudaStreamSynchronize(m_stream));   // needed for residual readback
            // Simple proxy: if max pressure change from last 10 iters is small, stop
            // (Full residual kernel would be more accurate but adds overhead)
            break;   // placeholder: always break after first 10 for now
                     // Replace with: if (residual < m_params.pressureTolerance) break;
        }
    }
}
