...
__global__ void mapParticleHeatSourcesKernel(
    float* heatSource, const float* fluidTemp,
    const float* particlePos, const float* particleTemp, int numParticles,
    int nx, int ny, int nz, float dx,
    float particleDensity, float particleSpecificHeat, float heatTransferCoeff,
    ...
    if (i >= 0 && i < nx && j >= 0 && j < ny && k >= 0 && k < nz) {
        ...
        float exchangeArea = dx * dx;
        float heatCapacity = fmaxf(particleDensity * particleSpecificHeat * cellVolume, 1e-12f);
        float dT = particleTemp[pid] - fluidTemp[idx];
        float sourceRate = heatTransferCoeff * exchangeArea * dT / heatCapacity;
...
__global__ void mapHeatSourcesKernel(
    float* heatSource, const float* fluidTemp,
    const float* nodePos, const float* nodeTemp, int numNodes,
    int nx, int ny, int nz, float dx,
    ...
    if (i >= 0 && i < nx && j >= 0 && j < ny && k >= 0 && k < nz) {
        ...
        float cellVolume = dx * dx * dx;
        float exchangeArea = dx * dx;
        float heatCapacity = fmaxf(fluidDensity * fluidSpecificHeat * cellVolume, 1e-12f);
        float dT = nodeTemp[pid] - fluidTemp[idx];
        float sourceRate = heatTransferCoeff * exchangeArea * dT / heatCapacity;
        atomicAdd(&heatSource[idx], sourceRate);
    }
}
...
void CFDSolverGPU::solvePressure() {
    ...
    for (int iter = 0; iter < m_params.maxPressureIters; ++iter) {
        // Save current pressure for residual computation
        if ((iter + 1) % 10 == 0) {
        ...
        if ((iter + 1) % 10 == 0) {
            // Compute L∞ residual: max|p_new - p_old|
            ...
            CUDA_CHECK_KERNEL();
            float maxDiff = 0.0f;
            CUDA_CHECK(cudaMemcpyAsync(&maxDiff, d_residual, sizeof(float),
                                       cudaMemcpyDeviceToHost, m_stream));
            
            // Use the max velocity kernel to find max|p - pOld| by repurposing
            // the reduction. Compute difference in-place on d_pOld.
            // Simple approach: sync and check on CPU
            CUDA_CHECK(cudaStreamSynchronize(m_stream));
            
#if 0
            // Sample a few cells for convergence proxy (avoids full reduction)
            int sampleIdx[4] = {
                m_totalCells / 4,
                m_totalCells / 2, 
                m_totalCells * 3 / 4,
            ...
            float sampledMaxDiff = 0;
            for (int s = 0; s < 4; ++s) {
                float pNew, pOld;
                cudaMemcpy(&pNew, &d_p[sampleIdx[s]], sizeof(float), cudaMemcpyDeviceToHost);
                cudaMemcpy(&pOld, &d_pOld[sampleIdx[s]], sizeof(float), cudaMemcpyDeviceToHost);
                float diff = fabsf(pNew - pOld);
                if (diff > sampledMaxDiff) sampledMaxDiff = diff;
            }
#endif
            
            if (maxDiff < m_params.pressureTolerance) {
                if (m_currentStep % 100 == 0) {
                    std::cout << "[CFD] Pressure converged at iter " << (iter+1)
...
void CFDSolverGPU::setSolidObstacles(const double* particlePositions, int numParticles) {
    ...
    // Volume-fraction IB: Gaussian kernel distribution (replaces binary bool)
    ...
    // Normalise: clamp accumulated fractions to [0, 1]
    // Simple per-element clamp kernel
...
void CFDSolverGPU::setHeatSources(const double* nodeTemperatures, const double* nodePositions, int numNodes) {
    ...
    CUDA_CHECK(cudaMemset(d_heatSource, 0, m_totalCells * sizeof(float)));
    ...
    float* d_nodeTemp = nullptr;
    float* d_nodePos = nullptr;
    CUDA_CHECK(cudaMalloc(&d_nodeTemp, numNodes * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_nodePos, numNodes * 3 * sizeof(float)));
    
    CUDA_CHECK(cudaMemcpy(d_nodeTemp, fTemp.data(), numNodes * sizeof(float), cudaMemcpyHostToDevice));
...
void CFDSolverGPU::setParticleHeatSources(
    ...
    std::vector<float> fTemp(numParticles);
    for (int i = 0; i < numParticles; ++i) fTemp[i] = (float)particleTemperatures[i];

    std::vector<float> fPos(numParticles * 3);
    for (int i = 0; i < numParticles * 3; ++i) fPos[i] = (float)particlePositions[i];

    float* d_particleTemp = nullptr;
    float* d_particlePos = nullptr;
    CUDA_CHECK(cudaMalloc(&d_particleTemp, numParticles * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_particlePos, numParticles * 3 * sizeof(float)));

    CUDA_CHECK(cudaMemcpy(d_particleTemp, fTemp.data(), numParticles * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_particlePos, fPos.data(), numParticles * 3 * sizeof(float), cudaMemcpyHostToDevice));
    ...
    mapParticleHeatSourcesKernel<<<blocks, threads, 0, m_stream>>>(
        ...
        m_params.nx, m_params.ny, m_params.nz, m_params.dx,
        (float)particleDensity, (float)particleSpecificHeat, m_params.heatTransferCoeff,
        0.0f, 0.0f, 0.0f);
    CUDA_CHECK_KERNEL();
...
std::vector<float> CFDSolverGPU::getTemperatureGrid() const {
    std::vector<float> T_host(m_totalCells);
