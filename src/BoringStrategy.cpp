...
void BoringStrategy::applyKinematics(double dt) {
    if (m_fem) {
        // Rotate tool around bore centerline
        ...
        // G-Code-driven position
        if (m_conditions.isGCodeDriven) {
            Vec3 delta = m_spindlePosition - m_prevSpindlePosition;
            if (delta.lengthSq() > 1e-18) {
                m_fem->translateMesh(delta.x, delta.y, delta.z);
            }
            m_conditions.toolPosition.z = m_spindlePosition.z;
        } 
        // Legacy: axial feed without G-Code
        else {
