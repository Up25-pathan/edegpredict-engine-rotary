...
void CoolantHardeningModel::update(MPMSolver* mpm, FEMSolver* fem, CFDSolverGPU* cfd, double dt) {
    ...
    if (m_params.filmBoilingEnabled) {
        double avgSurfaceTemp = mpm ? mpm->getAvgSurfaceTemp() : 25.0;
...
void CoolantHardeningModel::applyCFDCoolingToParticles(MPMSolver* mpm, CFDSolverGPU* cfd, double dt) {
    ...
    int n = mpm->getParticleCount();
    if (n == 0) return;

    // GPU path: launch device-side cooling kernel, bypassing all D2H copies
    MPMParticle* d_particles = mpm->getDeviceParticles();
    if (d_particles) {
        cfd->applyCFDCoolingDevice(d_particles, n, h, m_params.ambientTemperature, dt,
                                   m_state.lastCoolingApplied, m_state.lastHeatRemovedJ,
                                   m_state.cooledParticleCount);
    } else {
        // Fallback CPU path (should not be reached in normal operation)
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (Tp > 30.0) {
                ...
                double dT = Tp - Tc;
                if (dT > 1.0) {
                    double vol = fmax(particles[i].mass / fmax(particles[i].density, 1.0), 1e-24);
        ...
        mpm->setParticleTemperatures(newTemps.data(), n);
        m_state.lastCoolingApplied = totalCooled;
        m_state.lastHeatRemovedJ = totalHeatRemoved;
        m_state.cooledParticleCount = hotCount;
    }

    if (m_state.cooledParticleCount > 0 && m_state.updateCount % 500 == 0) {
        std::cout << "[CoolantHardening] Cooled " << m_state.cooledParticleCount << " particles, "
                  << "mean dT=" << (m_state.lastCoolingApplied / m_state.cooledParticleCount)
                  << " C, Qrem=" << std::scientific << std::setprecision(2)
                  << m_state.lastHeatRemovedJ << " J via " << h << " W/m2K HTC" << std::endl;
    }
}
