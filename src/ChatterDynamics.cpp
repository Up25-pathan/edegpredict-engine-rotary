...
void ChatterDynamics::solveRegenerativeChatter(double dt, double chipThickness) {
    ...
    int samples = 10;
    static double dispHistory[10] = {0};
    static int histIdx = 0;

    dispHistory[histIdx % samples] = currDisp;
    histIdx++;

    if (histIdx >= samples) {
        double mean = 0.0;
        for (int i = 0; i < samples; ++i) mean += dispHistory[i];
        mean /= samples;

        if (mean > 1e-12) {
            double variance = 0.0;
            for (int i = 0; i < samples; ++i)
                variance += (dispHistory[i] - mean) * (dispHistory[i] - mean);
            variance /= samples;
    ...
    if (growth > threshold && histIdx >= samples) {
    ...
    if (m_state.chattering && histIdx % 20 == 0) {
