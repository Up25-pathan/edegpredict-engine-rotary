...
bool SimulationEngine::step(double dt) {
    ...
    // --- Physics solvers ---
    // FIX: Skip SPH during air-cutting approach (saves massive computation)
    for (auto& solver : m_solvers) {
        if (m_skipSPHThisStep) {
            SPHSolver* sph = dynamic_cast<SPHSolver*>(solver.get());
            if (sph) continue;  // Skip this solver during air-cutting
        }
        solver->step(dt);
    }
    m_skipSPHThisStep = false;  // Reset flag after each step
