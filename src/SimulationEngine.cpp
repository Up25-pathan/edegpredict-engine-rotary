...
void SimulationEngine::handleAirCutting(double& dt) {
    ...
    double contactThreshold = m_contactSolver ? m_contactSolver->getConfig().contactRadius * 3.0 : 0.0015;
    double pad = 0.00005; // 50 microns padding
    
    // Normal physics takes over safely before the tool enters the contact detection radius.
    // The threshold is deliberately larger than the contact radius (3x) to allow SPH physics
    // to "wake up" and perform initial density relaxation without colliding with the tool.
    if (gap < contactThreshold + pad) return;
    ...
    if (m_gcodeInterpreter && m_gcodeInterpreter->isLoaded()) {
        ...
        // Use aggressively larger dt for bigger gaps.
        // ALL physics solvers are skipped during approach, so this is purely
        // advancing the G-Code interpolator — no CFL stability constraint.
        if (gap > 0.005) {
            macroDt = 0.1;       // 100ms for huge gaps (>5mm)
        } else if (gap > 0.002) {
            macroDt = 0.05;      // 50ms for large gaps (2-5mm)
        } else if (gap > contactThreshold + 0.0005) {
            macroDt = 0.01;      // 10ms (63 microns/step at 382mm/min)
        } else if (gap > contactThreshold + 0.00005) {
            macroDt = 0.001;     // 1ms (6.3 microns/step) - small enough to not overshoot padding
        } else {
            return;
        }
        ...
        //        CRITICAL: FEM stable dt ≈ 1e-8s, but macroDt can be 1ms = 100,000× CFL
        //        Running FEM with this dt causes node explosion ("crystal shatter")
