...
bool SimulationEngine::step(double dt) {
    ...
    if (m_gcodeInterpreter && m_gcodeInterpreter->isLoaded()) {
        ...
        for (auto& solver : m_solvers) {
            ...
            // Set FEM node velocities from G-Code so MPM collision kernel
            // reads correct tool velocities (avoids uninitialized GPU memory).
            fem->setAllNodeVelocities(velocity.x, velocity.y, velocity.z);
...
void SimulationEngine::handleAirCutting(double& dt) {
    ...
    // =====================================================================
    double gap = computeAirGap();
    if (!m_contactSolver || m_contactSolver->getContactCount() > 0 || gap <= 0) {
        if (gap <= 0) {
