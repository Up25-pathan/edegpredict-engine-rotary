    ...
    if (m_steadyStateReached && !m_extrapolationFired) {
        ...
        if (m_config.getSimulation().fidelityMode == "standard") {
            if (m_chatterDynamics && m_chatterDynamics->isEnabled() && m_chatterDynamics->isChattering()) {
                ...
                // Thermal extrapolation parameters
                double T_amb = m_config.getMachining().ambientTemperature;
                double tau = m_config.getSimulation().thermalTimeConstant;
                double projectedFinalTime = m_currentTime + remainingTime;

                // Tool temperature
                double projectedToolTemp = T_amb + (m_toolMaxTemperature - T_amb) * (1.0 - std::exp(-projectedFinalTime / tau));
                // Workpiece temperature
                double projectedWorkTemp = T_amb + (m_workpieceMaxTemperature - T_amb) * (1.0 - std::exp(-projectedFinalTime / tau));
                // Coolant temperature
                double projectedCoolantTemp = T_amb + (m_coolantMaxTemperature - T_amb) * (1.0 - std::exp(-projectedFinalTime / tau));
                ...
                std::cout << "[Extrapolation] Time constant tau:        " << tau << " s" << std::endl;
                std::cout << "============================================================\n" << std::endl;
