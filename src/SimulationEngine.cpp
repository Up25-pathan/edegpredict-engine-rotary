...
bool SimulationEngine::step(double dt) {
    ...
    if (m_strategy) {
        ...
        if (m_bueModel && m_bueModel->isEnabled()) {
            ...
            double shearStress = m_toolMaxStress;
    ...
    if (!m_skipPhysicsThisStep) {
        ...
        // ── Implicit predictor–corrector thermal coupling (Item 7) ──────────────
        ...
        double prevToolTemp = m_toolMaxTemperature;
        double prevWorkTemp = m_workpieceMaxTemperature;
        for (int iter = 0; iter < MAX_IMPLICIT_ITER; ++iter) {
            if (m_contactEngaged && m_contactSolver) {
                // 1. Blok/Jaeger heat partition
                {
                    double toolTemp = fmax(m_toolMaxTemperature, 25.0);
                    double workTemp = fmax(m_workpieceMaxTemperature, 25.0);
                ...
                if (m_coolantHardening) {
                    ...
                    // Also feed coolant HTC to FEM solver for convective BC (Item 4)
                    for (auto& solver : m_solvers) {
                        if (auto* fem = dynamic_cast<FEMSolver*>(solver.get())) {
            ...
            // 5. Convergence check — break if tool/workpiece temps stable
            double dTool = fabs(m_toolMaxTemperature - prevToolTemp);
            double dWork = fabs(m_workpieceMaxTemperature - prevWorkTemp);
            if (dTool < 1.0 && dWork < 1.0) break;
            prevToolTemp = m_toolMaxTemperature;
            prevWorkTemp = m_workpieceMaxTemperature;
        }
    }
...
void SimulationEngine::runMainLoop() {
    ...
    if (simParams.runToTargetDepth && m_config.getMachining().depthOfCutMm > 0.0) {
        ...
        const double stableDt = std::max(simParams.minTimeStep, computeAdaptiveTimeStep());
        if (feedRate > 1e-12 && stableDt > 1e-15) {
            ...
            std::cout << "[Engine] Depth-governed run enabled: target="
                      ...
                      << targetTime << " s, stable dt=" << std::scientific << stableDt
                      << " s, step budget=" << std::defaultfloat << totalSteps << std::endl;

            if (estimatedBudget > simParams.maxAutoExtendedSteps) {
                std::cerr << "[Engine] WARNING: target depth estimate exceeds max_auto_extended_steps="
                          << simParams.maxAutoExtendedSteps
                          << ". Increase mass scaling, coarsen MPM spacing, or raise the cap for full depth."
                          << std::endl;
            }
        }
    }
...
void SimulationEngine::exportResults() {
    ...
    // --- Tool R&D semantic report ---
    {
        ...
        rndInput.contactHeatJ = m_contactSolver ? m_contactSolver->getHeatGenerated() : 0.0;
