...
MachineStateSnapshot GCodeInterpreter::getStateAtTime(double simTime) const {
    ...
    state.feedRate = seg.feedRate * seg.cornerBlendFactor;
...
void GCodeInterpreter::computeBlendedFeedRates() {
    ...
    for (size_t i = 0; i < m_segments.size() - 1; ++i) {
        ...
        if (angle > ANGLE_THRESHOLD) {
            ...
            // Segment durations (timestamps) are kept canonical for the
            // single simulation clock — the blend factor is applied to the
            // feed rate snapshot returned by getStateAtTime() at the corner.
            next.cornerBlendFactor = std::max(0.1, std::cos(angle * 0.5));
        }
    }
