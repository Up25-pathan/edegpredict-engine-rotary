...
bool GCodeInterpreter::parseString(const std::string& gcodeContent) {
    ...
    // Fix 5: Post-processing pass — compute blended feed rates at segment
    // transitions to eliminate infinite jerk at sharp corners (G64 behavior).
    if (m_isLoaded && m_blendingEnabled) {
        computeBlendedFeedRates();
    }
...
void GCodeInterpreter::processGCommand(int gCode, double x, double y, double z, 
    ...
    switch (gCode) {
        ...
        case 61: // Exact stop mode (disable blending)
            m_blendingEnabled = false;
            break;
            
        case 64: // Continuous path mode (enable blending)
...
/**
 * @brief Post-processing pass: compute blended feed rates at segment transitions.
 *
 * For each pair of adjacent segments, computes the angle between the exit
 * direction of segment i and the entry direction of segment i+1.
 *
 * If the angle is sharp (> ~5°), the feed rate at the transition is reduced
 * proportionally: F_blend = F · cos(θ/2)
 *
 * This prevents infinite jerk/acceleration spikes at toolpath corners
 * that would generate false catastrophic stress in the FEM solver.
 *
 * Only active when m_blendingEnabled == true (set by G64).
 * G61 (exact stop) disables this.
 */
void GCodeInterpreter::computeBlendedFeedRates() {
    if (m_segments.size() < 2) return;

    const double ANGLE_THRESHOLD = 0.087;  // ~5 degrees in radians
    
    for (size_t i = 0; i < m_segments.size() - 1; ++i) {
        TrajectorySegment& curr = m_segments[i];
        TrajectorySegment& next = m_segments[i + 1];
        
        // Skip rapid moves — they don't produce cutting forces
        if (curr.motionType == 0 || next.motionType == 0) continue;
        
        // Compute exit direction of current segment
        Vec3 exitDir;
        if (curr.isArc) {
            // Tangent at end of arc
            double angle = curr.arcEndAngle;
            double sweepDir = (curr.arcEndAngle > curr.arcStartAngle) ? 1.0 : -1.0;
            exitDir = Vec3(
                -curr.arcRadius * std::sin(angle) * sweepDir,
                curr.arcRadius * std::cos(angle) * sweepDir,
                0).normalized();
        } else {
            Vec3 delta = curr.endPos - curr.startPos;
            exitDir = delta.lengthSq() > 1e-12 ? delta.normalized() : Vec3(0, 0, -1);
        }
        
        // Compute entry direction of next segment
        Vec3 entryDir;
        if (next.isArc) {
            double angle = next.arcStartAngle;
            double sweepDir = (next.arcEndAngle > next.arcStartAngle) ? 1.0 : -1.0;
            entryDir = Vec3(
                -next.arcRadius * std::sin(angle) * sweepDir,
                next.arcRadius * std::cos(angle) * sweepDir,
                0).normalized();
        } else {
            Vec3 delta = next.endPos - next.startPos;
            entryDir = delta.lengthSq() > 1e-12 ? delta.normalized() : Vec3(0, 0, -1);
        }
        
        // Angle between exit and entry
        double cosAngle = exitDir.dot(entryDir);
        cosAngle = std::max(-1.0, std::min(1.0, cosAngle));
        double angle = std::acos(cosAngle);
        
        if (angle > ANGLE_THRESHOLD) {
            // Blend factor: cos(θ/2) — smooth deceleration at corners
            // At 90° corner: cos(45°) = 0.707 → 70% feed
            // At 180° reversal: cos(90°) = 0 → minimum feed (clamped to 10%)
            double blendFactor = std::max(0.1, std::cos(angle * 0.5));
            
            // Apply blend: reduce feed at end of current and start of next
            // This is done by adjusting feed rates and recomputing segment times
            double blendedFeed = curr.feedRate * blendFactor;
            
            // Use the lower of the two segment feeds as base
            double baseFeed = std::min(curr.feedRate, next.feedRate);
            double cornerFeed = baseFeed * blendFactor;
            
            // Modify the segments: take a fraction of each and give it the 
            // blended feed rate. We encode this by averaging the segment feed
            // to approximate the trapezoidal profile.
            // Full implementation would split segments; this approximation
            // is sufficient to eliminate the discontinuity.
            curr.feedRate = (curr.feedRate + cornerFeed) * 0.5;
            next.feedRate = (next.feedRate + cornerFeed) * 0.5;
            
            // Recompute segment durations based on new feed rates
            Vec3 currDelta = curr.endPos - curr.startPos;
            double currDist = currDelta.length();
            if (curr.feedRate > 1e-12) {
                double newDuration = currDist / curr.feedRate;
                curr.endTime = curr.startTime + newDuration;
            }
            
            Vec3 nextDelta = next.endPos - next.startPos;
            double nextDist = nextDelta.length();
            next.startTime = curr.endTime;
            if (next.feedRate > 1e-12) {
                next.endTime = next.startTime + nextDist / next.feedRate;
            }
            
            // Cascade time shift to subsequent segments
            double timeShift = next.endTime - (i + 2 < m_segments.size() ? 
                               m_segments[i + 2].startTime : next.endTime);
            for (size_t j = i + 2; j < m_segments.size(); ++j) {
                m_segments[j].startTime += timeShift;
                m_segments[j].endTime += timeShift;
            }
        }
    }
    ...
    if (!m_segments.empty()) {
        m_totalDuration = m_segments.back().endTime;
    }
    
    std::cout << "[GCodeInterpreter] G64 velocity blending applied ("
