...
void GCodeInterpreter::parseLine(const std::string& line) {
    ...
    double x = m_currentState.absoluteMode ? (m_currentState.position.x / m_unitScale) : 0.0;
    double y = m_currentState.absoluteMode ? (m_currentState.position.y / m_unitScale) : 0.0;
    double z = m_currentState.absoluteMode ? (m_currentState.position.z / m_unitScale) : 0.0;
    double f = 0.0;
    ...
    bool hasI = false, hasJ = false, hasK = false, hasF = false;
    ...
    if (gCode >= 0) {
        processGCommand(gCode, x, y, z, f, iVal, jVal, kVal,
                        hasX, hasY, hasZ, hasI, hasJ, hasK, hasF);
...
void GCodeInterpreter::processGCommand(int gCode, double x, double y, double z, 
                                         ...
                                         bool hasI, bool hasJ, bool hasK, bool hasF) {
    switch (gCode) {
        case 0: // Rapid move
        ...
        case 1: // Linear interpolation
            ...
            m_pendingTarget = Vec3(x, y, z) * m_unitScale;
            if (hasF) {
                m_pendingFeed = f / 60.0 * m_unitScale;  // Convert file units/min to m/s
            }
            addSegment();
            break;
...
void GCodeInterpreter::computeBlendedFeedRates() {
    ...
    for (size_t i = 0; i < m_segments.size() - 1; ++i) {
        ...
        if (angle > ANGLE_THRESHOLD) {
            ...
            // Keep segment feed rates and timestamps canonical. A real G64
            // implementation needs a separate acceleration profile; mutating
            // segment durations here corrupts the single simulation clock.
    ...
    std::cout << "[GCodeInterpreter] G64 velocity blending analyzed ("
