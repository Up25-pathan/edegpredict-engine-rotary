...
MachiningOutput DrillingStrategy::computeOutput() const {
    ...
    double kappa = std::clamp(m_geometry.pointAngle * DEG_TO_RAD / 2.0,
                               DEG_TO_RAD * 0.5, DEG_TO_RAD * 44.5);
    // Chip compression factor (~2× for typical drilling) — should be calibrated
    // to material-specific shear angle data for production use.
    static constexpr double CHIP_COMPRESSION = 2.0;
    output.chipThickness = f_rev * std::sin(kappa) * CHIP_COMPRESSION;
