...
MachiningOutput DrillingStrategy::computeOutput() const {
    ...
    // f is in m/s → convert to mm/min: Vf (mm/min) = f * 60000
    // D is in m → convert to mm: D_mm = D * 1000
    // MRR = π/4 * D_mm² * Vf  (mm³/min)
    double Vf_mm_min = f * 60000.0;
    output.materialRemovalRate = (PI / 4.0) * (D * 1000.0) * (D * 1000.0) * Vf_mm_min;
