...
__global__ void applyAdiabaticHeatingKernel(SPHParticle* particles, int numParticles,
    ...
    if (p.strainRate < minThreshold) {
        ...
        p.residualStress = fmax(0.0, p.residualStress - 0.5);   // counter in residualStress
        return;
    }
    ...
    // We reuse p.residualStress as a float counter (0 to N steps).
    ...
    p.residualStress = fmin(p.residualStress + 1.0, 10.0);   // increment, cap at 10

    if (p.strainRate > params.criticalStrainRate && p.residualStress >= 3.0) {
