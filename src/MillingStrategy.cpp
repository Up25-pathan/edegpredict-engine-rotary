...
void MillingStrategy::connectSolvers(MPMSolver* sph, FEMSolver* fem, 
...
double MillingStrategy::computeChipLoadAtAngle(double angle) const {
    ...
    double omega = m_conditions.spindleSpeed;
    if (omega < 1e-6) omega = 1e-6;  // Guard: prevent division by zero
    double effectiveFeed = m_conditions.feedRate * m_adaptiveFeedMultiplier;
    double feedPerTooth = effectiveFeed / (m_geometry.numFlutes * 
                          omega / (2.0 * PI));  // m/tooth
...
std::vector<CuttingEdge> MillingStrategy::getActiveCuttingEdges() const {
    ...
    double radius = fmax(m_geometry.cutterDiameter, 1e-6) / 2.0;
...
MachiningOutput MillingStrategy::computeOutput() const {
    ...
    for (const auto& flute : m_flutes) {
        ...
        // Transform to global X-Y-Z
        totalCuttingForce.x += Ft * (-sinA) + Fr * cosA;
        totalCuttingForce.y += Ft * cosA + Fr * sinA;
        totalThrustForce.z += Fa;           // Axial force → thrust (was misassigned to feedForce.z)
        totalFeedForce.x += Ft * cosA - Fr * sinA;  // Feed component
    ...
    // MRR = ae * ap * Vf
    // Fix 10: Units — widthOfCut [m] * depthOfCut [m] * feedRate [m/s] = m³/s
    // Multiply by 1e9 to convert m³/s → mm³/s (1 m³ = 1e9 mm³)
...
void MillingStrategy::applyEdgeSubgridCorrections() {
    if (!m_edgeSubgrid || !m_edgeSubgrid->isEnabled()) return;

    double maxChipLoad = 0.0;
    for (const auto& flute : m_flutes) {
        maxChipLoad = std::max(maxChipLoad, flute.currentChipLoad);
    }

    if (maxChipLoad <= 0) return;

    auto result = m_edgeSubgrid->compute(
        maxChipLoad,
        m_specificCuttingForce * 0.5,
        m_conditions.cuttingSpeed,
        m_conditions.toolPosition.z > 0 ? 25.0 : 200.0,
...
void MillingStrategy::applyBUECorrections() {
...
void MillingStrategy::applyChatterModulation(double dt) {
