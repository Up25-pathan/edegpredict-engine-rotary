        ...
        if (q > Y && q > 1e-12) {
            ...
            if (deltaEps > 0) {
                ...
                double dT = config.taylorQuinney * Y * deltaEps /
                           (config.density * config.specificHeat);
                p.temperature += dT;
...
__device__ double atomicMinDouble(double* addr, double val) {
    ...
    do {
        assumed = old;
        if (__double_as_longlong(val) >= old) break;
...
bool MPMSolver::initialize(const Config& config) {
    ...
    // Initialize particle bounds to domain bounds
    double initMin[3] = {m_domainMin.x, m_domainMin.y, m_domainMin.z};
    double initMax[3] = {m_domainMax.x, m_domainMax.y, m_domainMax.z};
    ...
    m_particleMin = m_domainMin;
    m_particleMax = m_domainMax;
...
void MPMSolver::step(double dt) {
    ...
    double hBoundsMin[3] = { m_domainMin.x, m_domainMin.y, m_domainMin.z };
    double hBoundsMax[3] = { m_domainMax.x, m_domainMax.y, m_domainMax.z };
...
void MPMSolver::initializeParticleBox(const Vec3& minBounds, const Vec3& maxBounds, double spacing) {
    ...
    double density = m_mainConfig ? m_mainConfig->getMaterial().density : 7850.0;
