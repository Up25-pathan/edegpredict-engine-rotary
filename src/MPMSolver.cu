...
void MPMSolver::setDynamicMassScaling(double factor) {
    ...
    if (fabs(m_config.density - oldDensity) > 1e-6 && m_numParticles > 0) {
        ...
        // Note: CUDA_CHECK_KERNEL is omitted here to avoid missing macro scope issues, but standard sync applies
    }
}
...
void MPMSolver::syncMetrics() {
    ...
    cudaMemcpy(d_intEnergy, &initVal, sizeof(double), cudaMemcpyHostToDevice);

    int partThreads = 256;
    int partBlocks = (m_numParticles + partThreads - 1) / partThreads;
    computeParticleStatsKernel<<<partBlocks, partThreads>>>(
        d_particles, m_numParticles, d_maxStress, d_maxTemp, d_kinEnergy, d_intEnergy);
    CUDA_CHECK_KERNEL();
    ...
    cudaMemcpy(&m_internalEnergy, d_intEnergy, sizeof(double), cudaMemcpyDeviceToHost);
