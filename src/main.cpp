...
void generateParametricToolMesh(Mesh& mesh, const Config& config) {
    ...
    for (int row = 0; row <= nAxial; ++row) {
        double z = -(double)row / nAxial * length;
    ...
    for (int row = 0; row < nAxial; ++row) {
        for (int i = 0; i < nCirc; ++i) {
            int i2 = (i + 1) % nCirc;
            int a  = bodyStart + row * nCirc + i;
            int b  = bodyStart + row * nCirc + i2;
            int c  = bodyStart + (row + 1) * nCirc + i;
            int d  = bodyStart + (row + 1) * nCirc + i2;
            addTri(a, b, d);
            addTri(a, d, c);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    // RAII guard for IPC stdout redirect
    struct CoutRedirectGuard {
        std::streambuf* saved;
        bool active;
        CoutRedirectGuard(bool ipcEnabled) : saved(nullptr), active(ipcEnabled) {
            if (active) {
                saved = std::cout.rdbuf();
                std::cout.rdbuf(std::cerr.rdbuf());
                std::cerr << "[Main] IPC mode — stdout reserved for NDJSON" << std::endl;
            }
        }
        ~CoutRedirectGuard() {
            if (active && saved) std::cout.rdbuf(saved);
        }
    };
    CoutRedirectGuard coutGuard(ipcMode);
    ...
    try {
        ...
        {
            Config& config = engine.getMutableConfig();

            // ------------------------------------------------------------------
            // 2. Create physics solvers
            // ------------------------------------------------------------------
            auto mpmSolver = std::make_unique<MPMSolver>();
            ...
            if (!config.getFilePaths().workpieceGeometry.empty()) {
                Mesh workpieceMesh;
                if (geoLoader.load(config.getFilePaths().workpieceGeometry, workpieceMesh)) {
                    ...
                    double vol = (bb.max.x - bb.min.x) * (bb.max.y - bb.min.y) * (bb.max.z - bb.min.z);
                    int maxP = sphCfg.maxParticles;
                    double expectedCount = vol / (spacing * spacing * spacing);
                    if (expectedCount > maxP * 0.95) {
                        spacing = std::cbrt(vol / (maxP * 0.95));
                        config.getMutableSPH().smoothingRadius = spacing / sphCfg.particleSpacingFactor;
                        std::cout << "[Main] WARNING: CAD workpiece volume requires more particles than limit (" << maxP 
                                  << "). Auto-scaling spacing to " << spacing * 1000 << " mm.\n";
                    }
                    
                    mpmSolver->initialize(config);
                    mpmSolver->initializeParticleBox(bb.min, bb.max, spacing);
            ...
            if (!workpieceFromCAD) {
                // ================================================================
                // Smart default sizing: if user didn't specify dimensions,
                // auto-compute from tool/machining config so the workpiece is
                // always larger than the tool.
                // ================================================================
                // Drill diameter from config (10mm for DE10)
                double toolRadius_m = 0.005;  // 10mm dia / 2 default
                double depthOfCut   = config.getMachining().depthOfCutMm / 1000.0;
                
                if (useCylinder) {
                    // Config-driven or auto-sized dimensions
                    double wpRadius = wpGeo.radiusMm > 0 ? wpGeo.radiusMm / 1000.0 : toolRadius_m * 5.0;
                    double wpLength = wpGeo.lengthMm > 0 ? wpGeo.lengthMm / 1000.0 : depthOfCut * 1.5;
                    if (wpRadius < 0.010) wpRadius = 0.010;  // 10mm minimum
                    if (wpLength < 0.015) wpLength = 0.015;  // 15mm minimum
                    
                    double vol = 3.1415926535 * wpRadius * wpRadius * wpLength;
                    int maxP = sphCfg.maxParticles;
                    double expectedCount = vol / (spacing * spacing * spacing);
                    if (expectedCount > maxP * 0.95) {
                        spacing = std::cbrt(vol / (maxP * 0.95));
                        config.getMutableSPH().smoothingRadius = spacing / sphCfg.particleSpacingFactor;
                        std::cout << "[Main] WARNING: Cylinder workpiece volume requires more particles than limit (" << maxP 
                                  << "). Auto-scaling spacing to " << spacing * 1000 << " mm.\n";
                    }
                    
                    mpmSolver->initialize(config);
                    Vec3 center(0.0, 0.0, -wpLength / 2.0); 
                    mpmSolver->initializeCylindricalWorkpiece(center, wpRadius, wpLength, spacing, 2);
                    std::cout << "[Main] Workpiece: Cylinder R=" << wpRadius*1000 
                              << "mm L=" << wpLength*1000 << "mm" << std::endl;
                } else {
                    // Config-driven or auto-sized box dimensions
                    double sizeX = wpGeo.widthMm > 0 ? wpGeo.widthMm / 1000.0 : toolRadius_m * 10.0;
                    double sizeY = wpGeo.heightMm > 0 ? wpGeo.heightMm / 1000.0 : toolRadius_m * 10.0;
                    double sizeZ = wpGeo.depthMm > 0 ? wpGeo.depthMm / 1000.0 : depthOfCut * 1.5;
                    if (sizeX < 0.020) sizeX = 0.020;  // 20mm minimum
                    if (sizeY < 0.020) sizeY = 0.020;
                    if (sizeZ < 0.015) sizeZ = 0.015;
                    
                    double vol = sizeX * sizeY * sizeZ;
                    int maxP = sphCfg.maxParticles;
                    double expectedCount = vol / (spacing * spacing * spacing);
                    if (expectedCount > maxP * 0.95) {
                        spacing = std::cbrt(vol / (maxP * 0.95));
                        config.getMutableSPH().smoothingRadius = spacing / sphCfg.particleSpacingFactor;
                        std::cout << "[Main] WARNING: Box workpiece volume requires more particles than limit (" << maxP 
                                  << "). Auto-scaling spacing to " << spacing * 1000 << " mm.\n";
                    }
                    ...
                    mpmSolver->initializeParticleBox(workMin, workMax, spacing);
            ...
            // Convert to Volumetric Tet Mesh
            double targetVolumetricElementSize = 0.001; // 1mm
            GeometryLoader::voxelizeToTetMesh(toolMesh, targetVolumetricElementSize, config.getFEM().maxNodes);
            ...
            // Tool R&D semantic layer: classify real CAD mesh regions before
            // FEM initialization so region IDs propagate into contact, wear,
            // coating, VTK, and final aggregation.
            ToolRegionClassifier::classify(toolMesh, config);
            ...
            // contactRadius = 1.5 × h — reliable engagement without excessive false positives.
            // With proper tool positioning (not embedded), 1.5×h catches all particles
            // at the contact boundary without counting distant particles as contacts.
            contactCfg.contactRadius = sphCfg.smoothingRadius * 1.5;
            // k = E* × contactRadius  (reduced modulus, material vs tool)
            // E* ≈ 1/(((1-v1²)/E1) + ((1-v2²)/E2))
            double v1 = config.getMaterial().poissonsRatio;
            double E1 = config.getMaterial().youngsModulus;
            double v2 = config.getToolMaterial().poissonsRatio;
            double E2 = config.getToolMaterial().youngsModulus;
            double E_reduced = 1.0 / ((1.0-v1*v1)/E1 + (1.0-v2*v2)/E2);
            contactCfg.contactStiffness = E_reduced * sphCfg.smoothingRadius * 1.5;
            contactCfg.contactDampingRatio = 0.25;
            contactCfg.frictionCoefficient = 0.3;
            // Shear yield from material properties: τ_max = σ_yield / √3 (von Mises)
            contactCfg.workpieceShearYield = config.getMaterial().yieldStrength / 1.7320508;
            contactCfg.maxContactPressure = std::min(
                config.getToolMaterial().yieldStrength,
                std::max(1.0e9, 3.0 * config.getMaterial().yieldStrength));
            contactCfg.engagementRampSteps = 200;
            contactCfg.maxPenetrationFraction = 0.5;
            
            // Thermal properties from material definition
            contactCfg.toolSpecificHeat = config.getToolMaterial().specificHeat;
            contactCfg.workSpecificHeat = config.getMaterial().specificHeat;
            contactCfg.workPhysicalDensity = config.getMaterial().density;
            contactCfg.toolMeltTemp = config.getToolMaterial().meltingPoint;
            contactCfg.workMeltTemp = config.getMaterial().meltingPoint;
            
            std::cout << "[ContactSolver] Config: contactRadius=" << contactCfg.contactRadius*1000 
                      << "mm, E*=" << std::scientific << E_reduced 
                      << "Pa, τ_yield=" << contactCfg.workpieceShearYield/1e6 
                      << "MPa" << std::defaultfloat << std::endl;
            
            contactSolver.initialize(mpmSolver.get(), femSolver.get(), contactCfg);

            // ------------------------------------------------------------------
            // 3b. SDF-based contact (replaces spatial hash for primary CD)
            //     Build signed distance field from tool mesh at rest pose (θ=0).
            //     Runtime transform updates happen each step in the engine.
            // ------------------------------------------------------------------
            {
                bool sdfOk = contactSolver.initSDF(toolMesh);
                if (sdfOk) {
                    std::cout << "[Main] Tool SDF built: "
                              << toolMesh.triangles.size() << " tris → grid"
                              << std::endl;
                } else {
                    std::cout << "[Main] Tool SDF failed — falling back to spatial hash"
                              << std::endl;
                }
            }
            ...
            if (strategy) {
                ...
                strategy->connectSolvers(mpmSolver.get(), femSolver.get(),
                                         &contactSolver,
            ...
            // ------------------------------------------------------------------
            // 6b. Advanced physics models (edge subgrid, BUE, chatter)
            //     Use make_unique and pass raw pointers; unique_ptrs remain in
            //     scope until engine finishes execution.
            // ------------------------------------------------------------------
            auto edgeSubgrid = std::make_unique<EdgeSubgridModel>();
            edgeSubgrid->configure(config);
            auto bueModel = std::make_unique<BUEModel>();
            bueModel->configure(config);
            auto chatterDynamics = std::make_unique<ChatterDynamics>();
            chatterDynamics->configure(config);
            auto coolantHardening = std::make_unique<CoolantHardeningModel>();
            coolantHardening->configure(config);
            engine.setEdgeSubgridModel(edgeSubgrid.get());
            engine.setBUEModel(bueModel.get());
            engine.setCoolantHardeningModel(coolantHardening.get());
            engine.setChatterDynamics(chatterDynamics.get());
            ...
            // ------------------------------------------------------------------
            engine.addExporter(
                std::make_unique<VTKExporter>(config.getFilePaths().outputDirectory));
            ...
            // ------------------------------------------------------------------
            engine.addSolver(std::move(mpmSolver));
            ...
            std::cout << "\n[Main] Total execution time: "
                      ...
                      << elapsed.count() << " s\n";
            std::cout << "[Main] Workpiece max stress:      "
                      << engine.getMaxWorkpieceStress() / 1e6 << " MPa\n";
            std::cout << "[Main] Workpiece max temperature: "
                      << engine.getMaxWorkpieceTemperature() << " °C\n";
            std::cout << "[Main] Tool max stress:           "
                      << engine.getMaxToolStress() / 1e6 << " MPa\n";
            std::cout << "[Main] Tool max temperature:      "
                      << engine.getMaxToolTemperature() << " °C\n";
            std::cout << "[Main] Coolant max temperature:   "
                      << engine.getMaxCoolantTemperature() << " °C\n";
        }
