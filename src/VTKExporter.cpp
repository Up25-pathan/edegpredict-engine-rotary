...
void VTKExporter::exportReconstructedWorkpiece(const Mesh& mesh) {
    exportReconstructedWorkpiece(-1, 0.0, mesh);
}

void VTKExporter::exportReconstructedWorkpiece(int step, double time, const Mesh& mesh) {
    (void)time;
    std::string filename;
    if (step >= 0) {
        std::ostringstream ss;
        ss << m_outputDir << "/workpiece_" << std::setfill('0') << std::setw(6) << step << ".vtk";
        filename = ss.str();
    } else {
        filename = m_outputDir + "/workpiece_final.vtk";
    }
    ...
    exportMesh(filename, mesh, stress, temperature, wear, mesh.materialStatus);
    std::cout << "[VTKExporter] Exported reconstructed workpiece mesh to: " << filename << std::endl;
}
...
void VTKExporter::exportMesh(const std::string& filename, const Mesh& mesh,
                              ...
                              const std::vector<int>& materialStatus) {
    ...
    // Cells - Prioritize exact Surface Mesh (Triangles) for lightweight, high-fidelity rendering
    ...
    } else if (!mesh.elements.empty()) {
        // Fallback or purely volumetric exports
        int numCells = static_cast<int>(mesh.elements.size());
        file << "\nCELLS " << numCells << " " << (numCells * 5) << "\n";
        for (const auto& el : mesh.elements) {
            file << "4 " << el.nodeIndices[0] << " " << el.nodeIndices[1] << " " 
                 << el.nodeIndices[2] << " " << el.nodeIndices[3] << "\n";
        }
    ...
    if (!stress.empty() || !temperature.empty() || !wear.empty()) {
        ...
        file << "\nSCALARS ToolRegionId int 1\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& node : mesh.nodes) {
            file << static_cast<int>(node.toolRegion) << "\n";
        }

        file << "\nSCALARS ChippingRisk double 1\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& node : mesh.nodes) {
            file << std::fixed << std::setprecision(6) << node.chippingRisk << "\n";
        }

        file << "\nSCALARS CoatingRemaining_percent double 1\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& node : mesh.nodes) {
            file << std::fixed << std::setprecision(3)
                 << node.coatingRemaining * 100.0 << "\n";
        }

        file << "\nSCALARS FlankWear_um double 1\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& node : mesh.nodes) {
        ...
        file << "\nSCALARS CraterWear_um double 1\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& node : mesh.nodes) {
            file << std::scientific << std::setprecision(6)
                 << node.craterWear * 1e6 << "\n";
        ...
        // Material status (block vs chip segmentation)
        if (materialStatus.size() == mesh.nodes.size()) {
            file << "\nSCALARS MaterialStatus int 1\n";
            file << "LOOKUP_TABLE default\n";
            for (int ms : materialStatus) {
                file << ms << "\n";
            }
        }
