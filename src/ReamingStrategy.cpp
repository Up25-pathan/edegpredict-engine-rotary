...
void ReamingStrategy::connectSolvers(MPMSolver* sph, FEMSolver* fem, 
...
void ReamingStrategy::updateConditions(const MachineState& state, double dt) {
    ...
    if (rpm < 1.0) rpm = 1.0;
    if (rpm > 0) {
        double feedMmPerMin = state.feedRate * 60000.0 * m_adaptiveFeedMultiplier;
...
MachiningOutput ReamingStrategy::computeOutput() const {
    ...
    double R = fmax(m_reamDiameter, 1e-6) / 2.0;
        totalTorque += Ft * R;
    }
    ...
    // Cross-section = π * (R_out² - R_in²) = π * ( (D/2)² - (pilot/2)² )
    // MRR = cross-section * feed_rate
    double n_rps = m_conditions.spindleSpeed / (2.0 * PI);
    double R_outer = m_reamDiameter / 2.0;
    double R_inner = m_pilotHoleDiameter / 2.0;
    double crossSection = PI * (R_outer * R_outer - R_inner * R_inner);
    output.materialRemovalRate = crossSection * m_conditions.feedRate * n_rps * 1e9;
...
void ReamingStrategy::applyEdgeSubgridCorrections() {
    if (!m_edgeSubgrid || !m_edgeSubgrid->isEnabled()) return;
    double nRps = m_conditions.spindleSpeed / (2.0 * PI);
    double fz = (nRps > 0) ? m_conditions.feedRate / m_numFlutes : 0;
    if (fz <= 0) return;
    auto result = m_edgeSubgrid->compute(
        fz, m_specificCuttingForce * 0.5,
        m_conditions.cuttingSpeed,
        m_currentDepth > 0.001 ? 200.0 : 25.0,
        880e6, 113.8e9, 600e9);
...
void ReamingStrategy::applyBUECorrections() {
...
void ReamingStrategy::applyChatterModulation(double dt) {
