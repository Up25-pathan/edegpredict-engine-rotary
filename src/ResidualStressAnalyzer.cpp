...
void ResidualStressAnalyzer::analyzeFromParticles(const std::vector<MPMParticle>& particles,
    ...
    // Smoothing length estimate: assume it was set to ~1.5 particle spacings
    // For a 100-μm particle spacing, h ≈ 150 μm = 0.00015 m.
    // If not stored in the analyzer, use a conservative default.
    const double h_smooth = (m_smoothingLength > 0.0) ? m_smoothingLength : 0.00015;
    ...
    for (const auto& p : particles) {
        ...
        // ── SPH BOUNDARY COMPLETENESS CORRECTION ─────────────────────────────
        // Near the free surface, SPH particles lack neighbours on the air side.
        // Their density (and thus stress) is systematically underestimated by
        // the fraction of the support sphere that is occupied by material.
        //
        // Completeness estimate (linear ramp over one smoothing length):
        //   C(d) = min(1.0,  0.5 + d / h)
        //
        // Interpretation:
        //   d < h/2 : particle is at the surface; completeness ≈ 0.5 (half sphere)
        //   d = h   : particle has full sphere of support within material
        //   d > h   : fully interior; no correction needed
        //
        // Corrected stress = raw_stress / C(d)  (amplifies under-reported values)
        double completeness       = std::min(1.0, 0.5 + dist / h_smooth);
        double correctionFactor   = 1.0 / std::max(completeness, 0.3);  // min C=0.3 for safety

        // Compute von Mises stress
        double vonMises = computeVonMises(p.stress_xx, p.stress_yy, p.stress_zz,
                                           p.stress_xy, p.stress_xz, p.stress_yz);
        vonMises *= correctionFactor;   // apply boundary correction

        // Project stress to surface coordinates (with correction applied)
        double axial = (p.stress_xx * cuttingDir.x * cuttingDir.x +
                        p.stress_yy * cuttingDir.y * cuttingDir.y +
                        2 * p.stress_xy * cuttingDir.x * cuttingDir.y) * correctionFactor;

        double radial = (p.stress_xx * surfaceNormal.x * surfaceNormal.x +
                         p.stress_yy * surfaceNormal.y * surfaceNormal.y +
                         p.stress_zz * surfaceNormal.z * surfaceNormal.z) * correctionFactor;

        double feed = (p.stress_xx * feedDir.x * feedDir.x +
                       p.stress_yy * feedDir.y * feedDir.y +
                       p.stress_zz * feedDir.z * feedDir.z) * correctionFactor;
...
double ResidualStressAnalyzer::distanceToSurface(const MPMParticle& p, 
                                                   const Vec3& normal,
