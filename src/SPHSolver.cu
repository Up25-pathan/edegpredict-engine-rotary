...
__global__ void computeDensityPressureKernel(SPHParticle* particles, int numParticles,
    ...
    // Apply kernel normalization factor if needed or clamp density artificially near rest
    // to prevent initialization explosions from grid placement.
    // Allow slight variations to pass without generating GPa pressures instantly.
    double ratio = pi.density / config.restDensity;
    
    // Smooth out ratio near 1.0 to absorb initial grid layout noise (±2%)
...
void SPHSolver::step(double dt) {
    ...
    if (m_damageEnabled && (m_currentStep % 5 == 0)) {
        ...
        computeJCStressKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
            ...
            m_specificHeat, m_taylorQuinney, dt * 5.0);
        CUDA_CHECK_KERNEL();
