...
__device__ inline bool sphContinuumPair(ParticleStatus a, ParticleStatus b) {
    if (a == ParticleStatus::INACTIVE || b == ParticleStatus::INACTIVE) return false;
    if (a == ParticleStatus::CHIP || b == ParticleStatus::CHIP) return false;
    return true;
}
...
__global__ void separateChipsKernel(SPHParticle* particles, int numParticles,
                                     ...
                                     double meltTemp,
                                     double thermalSofteningChipRatio,
                                     double criticalPlasticStrain,
                                     bool chipDebondingEnabled) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numParticles) return;
    ...
    bool failedByDamage = p.damage >= damageThreshold;
    bool failedByStrain = p.plasticStrain >= criticalPlasticStrain;
    bool failedByThermalSoftening =
        meltTemp > 0.0 && p.temperature >= meltTemp * thermalSofteningChipRatio;

    if (failedByDamage || failedByStrain || failedByThermalSoftening) {
...
__global__ void computeStrainRateKernel(SPHParticle* particles, int numParticles,
    ...
    if (pi.status == ParticleStatus::CHIP) {
        return;
    }
        ...
        for (int dj = -1; dj <= 1; ++dj) {
            for (int dk = -1; dk <= 1; ++dk) {
                ...
                for (int j = start; j < end; ++j) {
                    ...
                    if (!sphContinuumPair(pi.status, pj.status)) continue;
...
/**
 * @brief Johnson-Cook constitutive stress tensor kernel.
 *
 * Fills the deviatoric stress tensor (stress_xx..yz) using the Levy-Mises
 * flow rule: the J-C scalar flow stress is distributed proportionally
 * to the deviatoric strain rate direction.
 *
 * σ_dev = (2/3 · σ_JC / max(ε̇_eq, ε̇_min)) · ε̇_dev
 * σ_total = σ_dev − P·I   (pressure from Tait EOS already stored)
 *
 * Also applies Taylor-Quinney plastic heating:
 *   ΔT = β · σ_JC · Δε_p / (ρ · Cp)
 *
 * Must be called AFTER computeStrainRateKernel (needs strainRate)
 * and computeDensityPressureKernel (needs pressure/density).
 */
__global__ void computeJCStressKernel(
    SPHParticle* particles, int numParticles,
    int* cellStart, int* cellEnd, int tableSize,
    SPHKernelConfig config, double cellSize,
    double domainMinX, double domainMinY, double domainMinZ,
    double jc_A, double jc_B, double jc_n, double jc_C, double jc_m,
    double refStrainRate, double refTemp, double meltTemp,
    double specificHeat, double taylorQuinney, double effectiveDt) {

    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numParticles) return;

    SPHParticle& pi = particles[idx];
    if (pi.status == ParticleStatus::INACTIVE ||
        pi.status == ParticleStatus::FIXED_BOUNDARY) return;

    // ── Recompute deviatoric strain rate tensor ──
    // We need the full tensor, not just the scalar stored on the particle.
    double dvx_dx = 0, dvx_dy = 0, dvx_dz = 0;
    double dvy_dx = 0, dvy_dy = 0, dvy_dz = 0;
    double dvz_dx = 0, dvz_dy = 0, dvz_dz = 0;

    int cx = (int)((pi.x - domainMinX) / cellSize);
    int cy = (int)((pi.y - domainMinY) / cellSize);
    int cz = (int)((pi.z - domainMinZ) / cellSize);
        ...
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                ...
                for (int j = start; j < end; ++j) {
                    ...
                    if (r2 < config.h2 && r2 > 1e-20) {
                        double r = sqrt(r2);
                        double diff = config.h - r;
                        double gradW = config.spikyGradCoeff * diff * diff / r;
                        double vol_j = pj.mass / fmax(pj.density, 1.0);
                        ...
                        dvx_dx += vol_j * dvx * dwx;
                        dvx_dy += vol_j * dvx * dwy;
                        dvx_dz += vol_j * dvx * dwz;
                        dvy_dx += vol_j * dvy * dwx;
                        dvy_dy += vol_j * dvy * dwy;
                        dvy_dz += vol_j * dvy * dwz;
                        dvz_dx += vol_j * dvz * dwx;
                        dvz_dy += vol_j * dvz * dwy;
                        dvz_dz += vol_j * dvz * dwz;
                    }
                }
            }
        }
    ...
    // ── Deviatoric strain rate (remove hydrostatic part) ──
    double trE = (exx + eyy + ezz) / 3.0;
    double dev_exx = exx - trE;
    double dev_eyy = eyy - trE;
    double dev_ezz = ezz - trE;

    // ── Equivalent strain rate ──
    double eps_dot_eq = sqrt(2.0/3.0 * (
        dev_exx*dev_exx + dev_eyy*dev_eyy + dev_ezz*dev_ezz +
        2.0*(exy*exy + exz*exz + eyz*eyz)));

    const double EPS_DOT_MIN = 1.0;  // 1/s floor
    double eps_dot_safe = fmax(eps_dot_eq, EPS_DOT_MIN);

    // ── Read current plastic strain (accumulated by computeDamageKernel) ──
    // NOTE: Do NOT accumulate here — computeDamageKernel is the single
    // canonical source of plastic strain increment to avoid double-counting.
    double d_eps_p = eps_dot_eq * effectiveDt;  // local increment for heating calc only
    double eps_p = fmax(pi.plasticStrain, 1e-10);

    // ── Johnson-Cook flow stress ──
    // σ = (A + B·εp^n) · (1 + C·ln(ε̇*)) · (1 − T*^m)

    double strainTerm = jc_A + jc_B * pow(eps_p, jc_n);

    double strainRateRatio = eps_dot_safe / fmax(refStrainRate, 1.0);
    if (strainRateRatio < 1.0) strainRateRatio = 1.0;
    double rateTerm = 1.0 + jc_C * log(strainRateRatio);

    double T_star = 0.0;
    if (pi.temperature > refTemp) {
        T_star = (pi.temperature - refTemp) / fmax(meltTemp - refTemp, 1.0);
        T_star = fmin(T_star, 1.0);
    }
    double thermalTerm = 1.0 - pow(T_star, jc_m);
    thermalTerm = fmax(thermalTerm, 0.01);
    ...
    pi.stress_yy = s_yy - pi.pressure;
    pi.stress_zz = s_zz - pi.pressure;
    pi.stress_xy = s_xy;
    pi.stress_xz = s_xz;
    pi.stress_yz = s_yz;

    // ── Taylor-Quinney plastic heating ──
    if (d_eps_p > 1e-15 && specificHeat > 0) {
        double dT = taylorQuinney * sigma_JC * d_eps_p /
                    (fmax(pi.density, 1.0) * specificHeat);
        pi.temperature = fmin(pi.temperature + dT, meltTemp);
    }
}
...
__global__ void computeDensityPressureKernel(SPHParticle* particles, int numParticles,
    ...
    if (pi.status == ParticleStatus::INACTIVE) return;
    if (pi.status == ParticleStatus::CHIP) {
        pi.density = config.restDensity * 0.1;
        pi.pressure = 0.0;
        return;
    }
    ...
    // Write back computed density (was using stale pi.density before)
    pi.density = fmax(density, config.restDensity * 0.1);  // Floor at 10% rest density
    ...
    // Clamp absolute pressure to avoid numerical explosions during early steps
    pi.pressure = fmax(-B * 0.5, fmin(pi.pressure, B * 0.5));
}
...
__global__ void leapfrogKickKernel(SPHParticle* particles, int numParticles, double halfDt) {
    ...
    // v += a * dt/2 (mass guard prevents NaN from zero-mass particles)
    double inv_mass = 1.0 / fmax(p.mass, 1e-15);
    double ax = p.fx * inv_mass;
    double ay = p.fy * inv_mass;
    double az = p.fz * inv_mass;
...
/**
 * @brief SPH inter-particle thermal conduction (Fourier's law via SPH Laplacian).
 *
 * Morris et al. (1997) formulation:
 *   dT_i/dt = SUM_j (2*k*m_j) / (rho_i*rho_j*Cp) * (T_i - T_j) / (r^2 + eps) * (r . gradW)
 *
 * This diffuses contact-generated heat from the surface into the bulk workpiece,
 * preventing infinite thermal gradients at the tool-workpiece interface.
 * Without this kernel, surface particles reach melting while interior stays at 25C.
 */
__global__ void computeThermalDiffusionKernel(
    SPHParticle* particles, int numParticles,
    int* cellStart, int* cellEnd, int tableSize,
    SPHKernelConfig config, double cellSize,
    double domainMinX, double domainMinY, double domainMinZ,
    double thermalConductivity, double specificHeat, double dt)
{
    ...
    // Skip particles at ambient — no heat to conduct
    if (pi.temperature <= 25.01) return;

    double dT = 0.0;
    double rho_i = fmax(pi.density, config.restDensity * 0.1);
        ...
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                ...
                for (int j = start; j < end; ++j) {
                    ...
                    double rz = pi.z - pj.z;
                    double r2 = rx*rx + ry*ry + rz*rz;
                    if (r2 >= config.h2 || r2 < 1e-20) continue;

                    double r = sqrt(r2);
                    double q = r / config.h;
                    
                    // Spiky gradient magnitude (same kernel as force computation)
                    double dWdr = config.spikyGradCoeff * (1.0-q)*(1.0-q) / r;

                    // r . gradW = |r|^2 * (dW/dr) / |r| = r * dW/dr
                    double rDotGradW = r * dWdr;

                    double rho_j = fmax(pj.density, config.restDensity * 0.1);
                    double Tij = pi.temperature - pj.temperature;

                    // Morris et al. (1997) SPH Laplacian heat conduction:
                    // dT_i/dt += (2*k * m_j) / (rho_i * rho_j * Cp) * T_ij / (r^2+eps) * (r.gradW)
                    double eps = 0.01 * config.h2;  // Singularity guard
                    double coeff = 2.0 * thermalConductivity * config.particleMass
                                 / (rho_i * rho_j * specificHeat * (r2 + eps));
                    dT -= coeff * Tij * rDotGradW;
                }
            }
        }
    }

    // Apply temperature change with stability clamp (max 50C per effective step)
    dT *= dt;
    dT = fmax(-50.0, fmin(50.0, dT));

    pi.temperature = fmax(25.0, pi.temperature + dT);  // Floor at ambient
}
...
__global__ void computeStepMaxVelocityKernel(SPHParticle* particles, int numParticles,
                                             double* maxVelocity) {
    __shared__ double s_maxVelocity[256];
...
bool SPHSolver::initialize(const Config& config) {
    ...
    m_chipDebondingEnabled = sphParams.chipDebondingEnabled;
    m_thermalSofteningChipRatio = sphParams.thermalSofteningChipRatio;
    m_criticalPlasticStrain = sphParams.criticalPlasticStrain;
    
    // Load J-C plasticity parameters for stress tensor computation
    m_jc_A = material.jc_A > 0 ? material.jc_A : 880e6;
    m_jc_B = material.jc_B > 0 ? material.jc_B : 290e6;
    m_jc_n = material.jc_n > 0 ? material.jc_n : 0.47;
    m_jc_C = material.jc_C > 0 ? material.jc_C : 0.015;
    m_jc_m = material.jc_m > 0 ? material.jc_m : 1.0;
    m_specificHeat = material.specificHeat > 0 ? material.specificHeat : 526.3;
...
void SPHSolver::allocateMemory(int capacity) {
    ...
    CUDA_CHECK(cudaMalloc(&d_stepMaxVelocity, sizeof(double)));
    ...
    CUDA_CHECK(cudaMemset(d_stepMaxVelocity, 0, sizeof(double)));
...
void SPHSolver::freeMemory() {
    ...
    if (d_stepMaxVelocity) { cudaFree(d_stepMaxVelocity); d_stepMaxVelocity = nullptr; }
...
void SPHSolver::initializeParticleBox(const Vec3& minCorner, const Vec3& maxCorner,
    ...
    if (m_mainConfig) {
        m_config.h = spacing / m_mainConfig->getSPH().particleSpacingFactor;
        double h = m_config.h;
        m_config.h2 = h * h;
        m_config.h3 = h * h * h;
        m_config.h6 = m_config.h3 * m_config.h3;
...
void SPHSolver::step(double dt) {
    ...
    // Step 7b: Thermal diffusion (every 5 steps — thermal timescale >> mechanical)
    // Fix E: SPH particles had no inter-particle heat conduction. Contact heat
    // stayed trapped at the surface, creating infinite thermal gradients.
    // Morris et al. (1997) SPH Laplacian formulation conducts heat into the bulk.
    if (m_currentStep % 5 == 0) {
        computeThermalDiffusionKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
            d_particles, m_numParticles,
            d_cellStart, d_cellEnd, m_hashTableSize,
            m_config, m_cellSize,
            m_domainMin.x, m_domainMin.y, m_domainMin.z,
            m_thermalConductivity, m_specificHeat, dt * 5.0);
        CUDA_CHECK_KERNEL();
    }
    NVTX_POP(); // Physics
    ...
    if (m_damageEnabled && (m_currentStep % 5 == 0)) {
        ...
        // Step 8b: Johnson-Cook constitutive stress tensor (Levy-Mises)
        // Fills stress_xx..yz — required by damage model, residual stress,
        // and surface roughness analysis.
        // Fix D: JC stress kernel must use same effective dt as damage kernel.
        // Both run every 5 steps, so the effective interval is dt * 5.0.
        // Previously JC stress used single-step dt while damage used dt*5,
        // causing damage to be 5× underestimated relative to stress.
        double effectiveDt = dt * 5.0;
        computeJCStressKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
            d_particles, m_numParticles,
            d_cellStart, d_cellEnd, m_hashTableSize,
            m_config, m_cellSize,
            m_domainMin.x, m_domainMin.y, m_domainMin.z,
            m_jc_A, m_jc_B, m_jc_n, m_jc_C, m_jc_m,
            m_refStrainRate, m_refTemp, m_meltTemp,
            m_specificHeat, m_taylorQuinney, effectiveDt);
        CUDA_CHECK_KERNEL();
        ...
        separateChipsKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
            d_particles, m_numParticles, m_damageThreshold, m_meltTemp,
            m_thermalSofteningChipRatio, m_criticalPlasticStrain,
            m_chipDebondingEnabled
        );
        CUDA_CHECK_KERNEL();
    ...
    CUDA_CHECK(cudaMemsetAsync(d_stepMaxVelocity, 0, sizeof(double), m_computeStream));
    computeStepMaxVelocityKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
        d_particles, m_numParticles, d_stepMaxVelocity);
    CUDA_CHECK_KERNEL();
    CUDA_CHECK(cudaMemcpyAsync(&m_lastStepMaxVelocity, d_stepMaxVelocity,
                               sizeof(double), cudaMemcpyDeviceToHost,
                               m_computeStream));
    cudaStreamSynchronize(m_computeStream);
    m_results.maxVelocity = m_lastStepMaxVelocity;
...
double SPHSolver::getStableTimeStep() const {
    ...
    double floorDt = m_mainConfig ? m_mainConfig->getSimulation().minTimeStep : 1e-12;
    return std::max(dt, floorDt);
}
