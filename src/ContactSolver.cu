...
void ContactSolver::initialize(MPMSolver* sph, FEMSolver* fem, const ContactConfig& config) {
    ...
    // Table size ≈ 2× node count, rounded up to next prime for uniform distribution
    ...
    m_hashTableSize = nextPrime(std::max(maxNodes * 2, 100003));
...
void ContactSolver::resolveContacts(double dt) {
    ...
    // Also applies radiation cooling (Item 5) and phase change (Item 6)
...
void ContactSolver::transferResults() {
    ...
    // Accumulate per-step heat into cumulative total (d_totalHeat is reset to 0
    // each step by cudaMemset in resolveContacts, so m_totalHeatGenerated is
    // per-step only). This cumulative value is used by the RND report export.
