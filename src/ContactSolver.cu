...
__global__ void xpbdContactSpatialHashKernel(
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        ...
        for (int j = start; j < end; ++j) {
            ...
            double contactArea = contactRadius * contactRadius * 3.14159265;
...
void ContactSolver::resolveContacts(double dt) {
    ...
    // Step 1: Build spatial hash for tool nodes (host-side, ~2K nodes = fast)
    {
        ...
        double dMinX = 1e10, dMinY = 1e10, dMinZ = 1e10;
        for (const auto& n : toolNodes) {
            dMinX = std::min(dMinX, (double)n.x);
            dMinY = std::min(dMinY, (double)n.y);
            dMinZ = std::min(dMinZ, (double)n.z);
        }
        dMinX -= m_cellSize * 5;
        dMinY -= m_cellSize * 5;
        dMinZ -= m_cellSize * 5;
