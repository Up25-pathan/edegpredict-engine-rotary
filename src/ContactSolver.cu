...
__global__ void applyNodeHeatAndHertzStressKernel(
    ...
    if (node.heatAccumulator > 0.0) {
        ...
        // BUG 5 FIX: Cap per-step temperature rise to 50°C
        // Prevents single-event thermal spikes (e.g. chip detachment burst)
        // from causing non-physical tool temperature jumps.
        double maxTempRise = 50.0;  // °C per step maximum
