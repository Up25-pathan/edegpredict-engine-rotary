...
// ============================================================================
// Spatial Hash Helpers (same convention as SPHSolver.cu)
// ============================================================================

__device__ inline int hashToolCell(int cx, int cy, int cz, int tableSize) {
    // Same prime-based hash as SPHSolver for consistency
...
__global__ void xpbdContactSpatialHashKernel(
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        ...
        for (int j = start; j < end; ++j) {
            ...
            if (dist >= contactRadius || dist < 1e-12) {
                if (isnan(dist)) {
                    // Try to print the NaN values
                    int prevCount = atomicAdd(contactCount, 1);
                    if (prevCount < 10) {
                        printf("[NaN DEBUG] ndx=%.5e, ndy=%.5e, ndz=%.5e, p=%.5e,%.5e,%.5e node=%.5e,%.5e,%.5e\n",
                               ndx, ndy, ndz, p.x, p.y, p.z, node.x, node.y, node.z);
                    }
                }
                continue;
            }

            int validPrev = atomicAdd(contactCount, 1);
            if (validPrev < 10) {
                printf("[VALID CONTACT] dist=%.5e, p.z=%.5e, node.z=%.5e\n", dist, p.z, node.z);
            }
            ...
            corrX += penetration * nx * engagementScale;
            corrY += penetration * ny * engagementScale;
            corrZ += penetration * nz * engagementScale;
            particleContactCount++;

            double fn = p.mass * penetration / dt2 * engagementScale;
            ...
            double heatThisStep = ft * vt * dt * engagementScale;
    ...
    // Velocity correction (impulse) - averaged over contacts to prevent Jacobi over-relaxation explosion
    double weight = 1.0 / particleContactCount;
    p.vx += (corrX * weight) / dt;
    p.vy += (corrY * weight) / dt;
    p.vz += (corrZ * weight) / dt;
...
__global__ void xpbdContactKernel(
    ...
    for (int j = 0; j < numNodes; ++j) {
        ...
        // Position correction vector (scaled by engagement ramp)
        corrX += penetration * nx * engagementScale;
        corrY += penetration * ny * engagementScale;
        corrZ += penetration * nz * engagementScale;
        particleContactCount++;
        ...
        double heatThisStep = ft * vt * dt * engagementScale;   // [J]
    ...
    // Averaged over contacts to prevent Jacobi over-relaxation explosion.
    double weight = 1.0 / particleContactCount;
    p.vx += (corrX * weight) / dt;
    p.vy += (corrY * weight) / dt;
    p.vz += (corrZ * weight) / dt;
...
void ContactSolver::resolveContacts(double dt) {
    ...
    // Step 1: Build spatial hash for tool nodes (host-side, ~2K nodes = fast)
    {
        ...
        for (int i = 0; i < numN; ++i) {
            int cx = (int)floor((toolNodes[i].x - dMinX) / m_cellSize);
            int cy = (int)floor((toolNodes[i].y - dMinY) / m_cellSize);
            int cz = (int)floor((toolNodes[i].z - dMinZ) / m_cellSize);
            int h = (cx * 73856093) ^ (cy * 19349663) ^ (cz * 83492791);
            toolNodes[i].cellHash = ((h % m_hashTableSize) + m_hashTableSize) % m_hashTableSize;
...
void launchContactInteraction(
    ...
    applyNodeHeatAndHertzStressKernel<<<gridSizeN, blockSizeN>>>(
        nodes, numNodes, contactRadius, 200.0, 2500.0, 0.9);
}
