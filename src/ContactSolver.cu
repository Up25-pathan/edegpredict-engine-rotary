...
__global__ void applyNodeHeatAndHertzStressKernel(
    ...
    if (node.heatAccumulator > 0.0 && hasPhaseChange) {
        ...
        if (T0 < Ts) {
            ...
            H0 = toolCp * Ts + latentHeatFusion * f;
        } else {
            H0 = toolCp * Ts + latentHeatFusion + toolCp * (T0 - Tl);
        }
        ...
        double Tnew;
        double H_s = toolCp * Ts;                  // enthalpy at solidus
        double H_l = toolCp * Ts + latentHeatFusion; // enthalpy at liquidus
        if (H1 <= H_s) {
            // Solid
            Tnew = H1 / toolCp;
        } else if (H1 < H_l) {
        ...
        double maxDelta = 50.0; // °C per step cap
        double dT = fmin(Tnew - T0, maxDelta);
        ...
        double maxTempRise = 50.0;
        dT = fmin(dT, maxTempRise);
