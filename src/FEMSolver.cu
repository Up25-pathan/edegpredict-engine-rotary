...
__global__ void thermalConductionTetKernel(
    ...
    for (int e = 0; e < 6; ++e) {
        ...
        if (a.mass > 1e-15 && specificHeat > 0) {
            atomicAdd(&a.temperature, -Q / (a.mass * specificHeat));
        }
        if (b.mass > 1e-15 && specificHeat > 0) {
            atomicAdd(&b.temperature,  Q / (b.mass * specificHeat));
        }
    }
}
...
__global__ void computeElementForcesKernel(FEMNodeGPU* nodes, int numNodes,
    ...
    // Using cross-product method (Müller et al. 2016):
    // R = F * (F^T F)^{-1/2}  approximated via 3 iterations of the
    // formula R_{k+1} = 0.5*(R_k + R_k^{-T})
    double R[3][3];
    // Initialize R = F
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
    ...
    // 3 iterations of R = 0.5*(R + R^{-T}) for polar extraction
    for (int iter = 0; iter < 3; ++iter) {
        // Compute cofactor matrix of R (= det(R) * R^{-T})
        double cof[3][3];
        ...
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
                R[i][j] = 0.5 * (R[i][j] + cof[j][i] * invDetR);  // note: cof transposed
    }
...
void FEMSolver::step(double dt) {
    ...
    // Fix 8: replaced static local with member m_typicalElementLength
    if (m_currentStep % 100 == 0 && m_numElements > 0) {
        m_typicalElementLength = pow(h_elements.front().volume, 1.0/3.0);
    }
    double typicalL = m_typicalElementLength;
    ...
    // Fix 3: Thermal conduction through tetrahedral element edges
    // Conducts heat along each edge pair (6 per tet) using Q̇ = k·A·ΔT/L
