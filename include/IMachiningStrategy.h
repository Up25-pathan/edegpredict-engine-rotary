...
struct ToolGeometry {
    ...
    double webThickness = std::numeric_limits<double>::quiet_NaN();        // m
...
struct CuttingConditions {
    ...
    double bueEffectiveRakeChangeDeg = 0.0;
    double bueCoverageFraction = 0.0;
    double bueForceModulation = 1.0;

    // Chatter Dynamics corrections
    double chipThicknessModulation = 0.0;
    double vibrationDisplacementUm = 0.0;
...
struct MachiningOutput {
    ...
    std::vector<MPMParticle> surfaceParticles;
    Vec3 surfaceNormal;
    double profileLength = 0;
    double toolNoseRadius = 0;
...
public:
     ...
     */
    virtual void connectSolvers(MPMSolver* sph, FEMSolver* fem, 
    ...
    /**
     * @brief Set edge subgrid model (tool edge hone / size effect)
     */
    virtual void setEdgeSubgridModel(EdgeSubgridModel* model) {}

    /**
     * @brief Get edge subgrid model result
     */
    virtual const EdgeSubgridModel* getEdgeSubgridModel() const { return nullptr; }

    // === BUE Model Interface ===
    /**
     * @brief Set built-up edge model
     */
    virtual void setBUEModel(BUEModel* model) {}

    /**
     * @brief Get BUE model state
     */
    virtual const BUEModel* getBUEModel() const { return nullptr; }

    // === Chatter Dynamics Interface ===
    /**
     * @brief Set chatter dynamics model
     */
    virtual void setChatterDynamics(ChatterDynamics* chatter) {}

    /**
     * @brief Apply edge subgrid corrections to cutting conditions
     */
    virtual void applyEdgeSubgridCorrections() {}

    /**
     * @brief Apply BUE corrections to cutting conditions
     */
    virtual void applyBUECorrections() {}

    /**
     * @brief Apply chatter modulation to chip thickness
     */
    virtual void applyChatterModulation(double dt) {}
};
