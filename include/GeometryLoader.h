...
public:
    ...
    /**
     * @brief Fills the hollow surface mesh with a volumetric Tetrahedral mesh
     * Uses a highly robust Cartesian grid voxelizer and 6-tet split algorithm.
     * @param mesh The mesh to modify (surface replaced by volume).
     * @param elementSize The edge length of the voxels (m).
     * @param maxNodes Maximum allowed nodes before scaling up elementSize.
     */
    static void voxelizeToTetMesh(Mesh& mesh, double elementSize, int maxNodes);
