...
public:
     ...
     */
    void analyzeFromParticles(const std::vector<MPMParticle>& particles,
...
private:
     ...
     */
    double distanceToSurface(const MPMParticle& p, const Vec3& normal, 
    ...
    /// SPH smoothing length h (m). Used for boundary completeness correction:
    ///   C(d) = min(1.0, 0.5 + d/h).
    /// Set from MPMSolver::m_config.h before calling analyzeFromParticles().
    double m_smoothingLength = 0.00015;   // 150 μm default
