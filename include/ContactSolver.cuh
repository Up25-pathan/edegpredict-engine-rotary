...
public:
    ...
    double getCumulativeHeatGenerated() const { return m_cumulativeHeatGenerated; }
    ...
    bool isSDFEnabled() const { return m_sdfEnabled; }
    cudaStream_t getComputeStream() const { return m_computeStream; }

    // SDF-based contact detection (public — called from engine/main)
    bool initSDF(const Mesh& toolMesh);
    void setToolTransform(float centerX, float centerY, float centerZ,
                          float cosAngle, float sinAngle);

    /**
     * @brief Morph the tool SDF geometry based on accumulated wear.
     * Computes worn surface positions by displacing each vertex along its
     * inward normal by the wear depth read from the FEM solver, then
     * rebuilds the GPU SDF texture to reflect the dulled tool shape.
     * @return true if SDF was updated
     */
    bool morphToolSDF();
...
private:
    ...
    // Shared spatial hash for O(N×k) contact detection
    SharedSpatialHash m_hash;

    // SDF-based contact detection (replaces hash for primary CD)
    ToolSDF m_sdf;
    bool m_sdfEnabled = false;

    // Tool SDF morphing state (for dynamic tool wear)
    std::vector<Vec3> m_sdfOriginalPositions;  // Rest-pose vertex positions passed to SDF
    std::vector<Triangle> m_sdfTriangles;      // Triangle connectivity for SDF
    std::vector<Vec3> m_sdfVertexNormals;      // Per-vertex averaged face normals

    // Per-solver CUDA stream for async operations
    cudaStream_t m_computeStream = nullptr;
    cudaEvent_t m_transferEvent = nullptr;
    ...
    double m_cumulativeHeatGenerated = 0;
...
// ── SDF Contact & Node Update Kernels ──────────────────────────────────
...
/// Mark FEM nodes as inContact when they are within the SDF narrow band.
