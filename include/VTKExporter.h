...
public:
    ...
    /**
     * @brief Export the reconstructed solid workpiece as a VTK mesh (indexed)
     * @param step Step number (for filename), use -1 for final export
     * @param mesh Reconstructed workpiece mesh
     * @param materialStatus Optional per-vertex material status (0=block, 1=chip, 2=air)
     */
    void exportReconstructedWorkpiece(const Mesh& mesh);
    void exportReconstructedWorkpiece(int step, double time, const Mesh& mesh) override;
