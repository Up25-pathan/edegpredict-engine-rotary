...
public:
    ...
    double getSmoothingRadius() const { return m_config.h; }
...
private:
    ...
    double* d_stepMaxVelocity = nullptr;
    ...
    double m_lastStepMaxVelocity = 0.0;
    ...
    bool m_chipDebondingEnabled = true;
    double m_thermalSofteningChipRatio = 0.98;
    double m_criticalPlasticStrain = 2.0;
    
    // Johnson-Cook constitutive model (plasticity — stress tensor computation)
    double m_jc_A = 880e6;     // Yield strength (Pa)
    double m_jc_B = 290e6;     // Strain hardening coefficient (Pa)
    double m_jc_n = 0.47;      // Strain hardening exponent
    double m_jc_C = 0.015;     // Strain rate sensitivity
    double m_jc_m = 1.0;       // Thermal softening exponent
    double m_specificHeat = 526.3;  // J/(kg·K)
    double m_taylorQuinney = 0.9;   // Fraction of plastic work → heat
    double m_thermalConductivity = 6.7; // W/(m·K) for inter-particle conduction
