...
struct SimulationParams {
    std::string fidelityMode = "standard"; // "standard" or "scientific"
    int numSteps = 1000;
    ...
    bool dynamicReconstruction = false;  // Reconstruct workpiece surface + CSG carve at every output interval
    ...
    // Frequency-domain AMS — critical-damping-informed limit
    bool   amsFrequencyDomainEnabled = true;    // Enable frequency-domain critical damping target
    double amsFrequencyTarget = 0.0;            // Target frequency (Hz); 0 = auto from CFL
    double amsSpectralRadiusLimit = 0.99;       // Max spectral radius before reducing CFL (0-1)
    double amsAdaptiveCFLMin = 0.05;            // Minimum CFL when spectral radius is exceeded
    double amsAdaptiveCFLMax = 0.5;             // Maximum CFL when spectral radius is safe
    ...
    double thermalTimeConstant = 5.0;   // seconds, for steady-state temp extrapolation

    // Implicit thermal coupling convergence
    int maxThermalIterations = 8;        // Max iterations for the predictor–corrector loop
    double thermalConvergenceTolerance = 0.5;  // °C — stop when max node temp delta < this
    bool enableThermalConvergenceLogging = true; // Print iteration count + residual each step
};
...
/**
 * @brief MPM solver parameters (Material Point Method — replaces legacy SPH naming)
 * @note Accessor methods retain `getSPH()` naming for backward compatibility.
 */
struct MPMParams {
...
struct CFDParams {
    ...
    bool chipFluidCouplingEnabled = true;    // Enable chip drag + IB velocity BC
};
