...
private:
    ...
    /**
     * @brief Crater wear rate using Usui diffusion model.
     *
     * dKT/dt = A_usui * σ * v * exp(-ΔH / (R * T_K))
     *
     * Reference: Usui, Shirakashi, Kitagawa, ASME J. Eng. Ind. 100 (1978) 236-243.
     * WC-Co on steel: A_usui = 1e-3, ΔH/R = 16600 K.
     */
    double computeUsuidiffusionRate(double stress, double velocity,
    ...
    double m_flankWearCoeff  = 1e-12;   // Archard K for flank/cutting edge wear
    double m_craterWearCoeff = 5e-13;   // (legacy; crater now uses Usui model)
    // Note: Usui model constants are embedded in computeUsuidiffusionRate().
};
