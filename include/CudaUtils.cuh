...
// ============================================================================
// Atomic Operations for double
// ============================================================================

/**
 * @brief Atomic max for double (CUDA does not natively provide this)
 * 
 * Uses atomicCAS on the underlying 64-bit integer representation.
 * Safe on all CUDA architectures where sizeof(double) == sizeof(unsigned long long).
 */
#ifdef __CUDACC__
__device__ inline double atomicMaxDouble(double* addr, double value) {
    static_assert(sizeof(double) == sizeof(unsigned long long),
                  "atomicMaxDouble requires 64-bit double");
    unsigned long long* addrAsULL = reinterpret_cast<unsigned long long*>(addr);
    unsigned long long old = *addrAsULL;
    unsigned long long assumed;
    do {
        assumed = old;
        if (value <= __longlong_as_double(assumed)) break;
        old = atomicCAS(addrAsULL, assumed, __double_as_longlong(value));
    } while (assumed != old);
    return __longlong_as_double(old);
}

/**
 * @brief Atomic add for double (safe wrapper, CUDA 2.0+ supports this natively)
 */
__device__ inline double atomicAddDouble(double* addr, double value) {
    return atomicAdd(addr, value);
}

/**
 * @brief Atomic min for double
 */
__device__ inline double atomicMinDouble(double* addr, double val) {
    ...
    do {
        ...
        old = atomicCAS(addrAsULL, assumed, __double_as_longlong(val));
    } while (assumed != old);
    return __longlong_as_double(old);
}
#endif
